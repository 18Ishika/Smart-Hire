...
function Upload() {
  ...
  // ── email state ──────────────────────────────────────────────
  const [emailModal, setEmailModal] = useState(null);   // resume object | null
  const [emailRound, setEmailRound] = useState("default");
  ...
  const openEmailModal = (resume) => {
    setEmailModal(resume);
    setEmailRound("default");
    setEmailMessage("");
    setEmailStatus(null);
  };
  ...
  const handleSendEmail = async () => {
    ...
    setEmailStatus(null);
    try {
      const body = { round: emailRound };
      if (emailMessage.trim()) body.message = emailMessage.trim();

      await api.post(`/api/resumes/${emailModal.id}/send-email/`, body);
      setEmailStatus({ ok: true, text: "Email sent successfully!" });
    } catch (err) {
  ...
  const ROUND_LABELS = [
    { value: "default", label: "General invitation" },
    ...
    <div className="upload-page">
            ...
            <div className="file-list" style={{ marginTop: "12px", display: "flex", flexDirection: "column", gap: "6px" }}>
              {files.map((f, i) => (
                <span key={i} className="file-chip"
                  style={{ fontSize: "12px", padding: "5px 10px", background: "#e8f6f7",
                    color: "#1f6a6b", borderRadius: "8px", overflow: "hidden",
        ...
        <div className="report-box">
          {sortedResumes.length === 0 ? (
            <div className="empty-state"
              style={{ textAlign: "center", padding: "60px 20px", color: "#7ba8aa",
                fontSize: "14px", background: "#fff", borderRadius: "14px",
                ...
                {currentResumes.map((r, index) => {
                  ...
                  return (
                    <div key={r.id} className="resume-card">
                      ...
                      <p style={{ fontSize: "14px", fontWeight: 600, color: "#1a3a3c",
                      ...
                      {r.status === "Processed" && (
                        <>
                          <button className="explain-btn"
                            ...
                            style={{ marginTop: "6px" }}>
                            {loadingExplain[r.id] ? "Loading..."
                              : explanations[r.id] ? "Hide explanation"
                              : "Why this rank?"}
                          </button>
                          ...
                          <button className="explain-btn"
                            ...
                            style={{ marginTop: "6px", color: "#1f6a6b", borderColor: "#1f6a6b" }}>
                            Send invite →
                          </button>
                        </>
                      )}
      ...
      {selectedPdf && (
        <div className="pdf-modal"
          ...
          <div className="pdf-container"
            style={{ background: "#fff", borderRadius: "18px", width: "100%",
              maxWidth: "860px", height: "90vh", display: "flex",
              flexDirection: "column", padding: "20px", gap: "12px" }}>
            ...
            {pdfLoading && <p style={{ fontSize: "13px", color: "#7ba8aa" }}>Loading PDF...</p>}
            <iframe src={`${selectedPdf}#toolbar=1`} title="Resume Viewer"
      ...
      {emailModal && (
        <div style={{ position: "fixed", inset: 0, zIndex: 1100,
          ...
          alignItems: "center", justifyContent: "center", padding: "24px" }}>
          <div style={{ background: "#fff", borderRadius: "18px", width: "100%",
            maxWidth: "480px", padding: "28px", display: "flex",
            flexDirection: "column", gap: "16px" }}>

            {/* header */}
            <div>
              <p style={{ fontSize: "11px", color: "#7ba8aa", margin: "0 0 4px", textTransform: "uppercase", letterSpacing: "0.5px" }}>
                Send interview invite
              </p>
              <h3 style={{ margin: 0, fontSize: "15px", color: "#1a3a3c", fontWeight: 600 }}>
                {emailModal.actual_resume_file_name}
              </h3>
            </div>

            {/* round picker */}
            <div>
              <label style={{ fontSize: "12px", color: "#5a8688", display: "block", marginBottom: "6px" }}>
                Interview round
              </label>
              <select
                value={emailRound}
                onChange={(e) => setEmailRound(e.target.value)}
                style={{ width: "100%", padding: "8px 10px", borderRadius: "8px",
                  border: "1px solid #b2d8d8", fontSize: "13px", color: "#1a3a3c",
                  background: "#f4fbfb", outline: "none" }}>
                {ROUND_LABELS.map((r) => (
            ...
            <div>
              <label style={{ fontSize: "12px", color: "#5a8688", display: "block", marginBottom: "6px" }}>
                Custom message <span style={{ opacity: 0.5 }}>(optional — leave blank to use default)</span>
              </label>
              <textarea
                rows={4}
                value={emailMessage}
                onChange={(e) => setEmailMessage(e.target.value)}
                placeholder="Leave empty to send the default invitation email…"
                style={{ width: "100%", padding: "8px 10px", borderRadius: "8px",
                  border: "1px solid #b2d8d8", fontSize: "13px", color: "#1a3a3c",
                  background: "#f4fbfb", resize: "vertical",
            ...
            {emailStatus && (
              <p style={{ margin: 0, fontSize: "13px",
                color: emailStatus.ok ? "#0f6e56" : "#993c1d",
                background: emailStatus.ok ? "#e1f5ee" : "#faece7",
            ...
            <div style={{ display: "flex", gap: "10px", justifyContent: "flex-end" }}>
              ...
              <button className="upload-btn"
                ...
                style={{ width: "auto", padding: "9px 22px", marginTop: 0,
